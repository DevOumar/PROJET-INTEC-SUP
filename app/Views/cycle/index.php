    ...
    <div class="col-xl-4">
        <div class="card">
            <div class="card-header bg-primary d-flex justify-content-between">
                ...
                <a href="<?= base_url('cycles') ?>" class="btn btn-rounded btn-info btn-sm">Recharger</a>
            </div>
            <div class="card-body">
                <form method="POST" <?php if (isset($cycle)): ?> action="<?= base_url('cycles/index/' . $cycle->id) ?>"
                    <?php else: ?> action="<?= base_url('cycles') ?>" <?php endif; ?>>
                    <?= csrf_field() ?>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group has-feedback">
                                <label class="control-label">Libellé</label>
                                <input type="text" name="libelle" value="<?= isset($cycle) ? $cycle->libelle : '' ?>"
    ...
    <div class="col-xl-8">
        <div class="card">
            <div class="card-header bg-primary">
                <h4 class="text-white m-b-0">Liste des cycles(<?php echo count($cycles); ?>)</h4>
            </div>
            <div class="card-body">
                <hr>
                        ...
                        <tbody>
                            <?php foreach ($cycles as $key => $cycle): ?>
                                <tr>
                                    <td><?= $key + 1 ?></td>
                                    <td><span
                                            class="btn btn-rounded btn-primary btn-sm"><?= mb_strtoupper($cycle->libelle, 'UTF-8') ?></td></span>
                                    </td>
                                    <td><span
                                            class="btn btn-rounded btn-success btn-sm"><?= date('d/m/Y à H:i', strtotime($cycle->created_at)) ?></span>
                                    </td>
                                    <td>
                                        <a href="<?= base_url('cycles/index/' . $cycle->id) ?>"><i
                                                class="fa fa-edit"></i></a>
                                        <a href="#" class="supelm" data-id="<?= $cycle->id ?>"><i

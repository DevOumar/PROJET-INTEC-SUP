    ...
    <div class="col-xl-4">
        <div class="card">
            <div class="card-header bg-primary d-flex justify-content-between">
                ...
                <a href="<?= base_url('casiers') ?>" class="btn btn-rounded btn-info btn-sm">Recharger</a>
            </div> 
<div class="card-body">
    <form method="POST" <?php if (isset($casier)): ?> action="<?= base_url('casiers/index/' . $casier->id) ?>" <?php else: ?> action="<?= base_url('casiers') ?>" <?php endif; ?>>
        <?= csrf_field() ?>
        <div class="row">
            <div class="col-md-12">
                <div class="form-group has-feedback">
                    <label class="control-label">Libellé</label>
                    <input type="text" name="libelle" value="<?= isset($casier) ? $casier->libelle : '' ?>" class="form-control" placeholder="Libellé">
...
<div class="col-xl-8">
    <div class="card">
        <div class="card-header bg-primary">
            <h4 class="text-white m-b-0">Liste des casiers(<?= count($casiers); ?>)</h4>
        </div>
        <div class="card-body">
            <hr>
            <div class="table-responsive">
                <table id="dataList" class="table table-bordered table-hover" data-name="cool-table">
                    ...
                    <tbody>
                        <?php foreach ($casiers as $key => $casier): ?>
                            <tr>
                                <td><?= $key + 1 ?></td>
                                <td><span class="btn btn-rounded btn-primary btn-sm"><?= strtoupper($casier->libelle) ?></span></td>
                                <td><span class="btn btn-rounded btn-success btn-sm"><?= date('d/m/Y à H:i', strtotime($casier->created_at)) ?></span></td>
                                <td>
                                    <a href="<?= base_url('casiers/index/' . $casier->id) ?>"><i class="fa fa-edit"></i></a>
...
$('body').on('click', '.supelm', function (e) {
    ...
    $.ajax({
        url: "<?= base_url('casiers/delete/') ?>" + id,
       // type: 'post',
        cache: false,
        async: true
    })
